...
distributions = { 'j'+str(j+1): truncate_and_round(np.random.normal(clients[j]['expD'], clients[j]['devD'], 1000000)) for j in range(3) }
...
labels = [ 'reg ' + ck for ck in class_keys ] + [ 'descr' + ck for ck in class_keys ] + [ 'act' + ck for ck in class_keys ]
labels_reordered = [ classname + ' ' + dtype for classname,dtype in itertools.product(class_keys,['reg', 'descr', 'act']) ]
